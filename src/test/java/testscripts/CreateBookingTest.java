...
//		System.out.println(res.getStatusLine());
	...
	public void createBookingTestWithPOJO() {
		...
		payload.setFirstname("Rashmi");
		payload.setLastname("Ganachari");
	...
	public void updateBookingIdTest() {
		...
		// bookingId=2522;
		payload.setFirstname("Shamith");

		Response res = RestAssured.given().headers("Content-Type", "application/json")
				.headers("Accept", "application/json").headers("Cookie", "token=" + token).log().all().body(payload)
	...
	public void partialUpdateBookingIdTest() {
		...
		payload.setFirstname("Reena");
		payload.setLastname("Patil");

		Response res = RestAssured.given().headers("Content-Type", "application/json")
				.headers("Accept", "application/json").headers("Cookie", "token=" + token).log().all()
				.body("{\r\n" + "    \"firstname\" : \"Reena\",\r\n" + "    \"lastname\" : \"Patil\"\r\n" + "}").when()
	...
	public void deleteBookingIdTest() {
		...
		Response getAllBookingIdRes = RestAssured.given().headers("Accept", "application/json").log().all().when()
				.get("/booking");

		List<Integer> listOfBookingIds = getAllBookingIdRes.jsonPath().getList("bookingid");
